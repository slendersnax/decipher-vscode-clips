...
class makeRowsLabelMatchValueCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        try:
            sels = self.view.sel()
            input = ''
...
class makeChoicesLabelMatchValueCommand(sublime_plugin.TextCommand):
    def run (self,edit):
        try:
            ...
            for sel in sels:
                ...
                count = 0
                for x in input:
                    printPage += "  <choice label=\"ch%s\" value=\"%s\">%s</choice>\n" % (str(count+1), str(count+1), input[count].strip())
                    count += 1
            self.view.replace(edit, sel, printPage)
        except Exception as e:
            print (e)
